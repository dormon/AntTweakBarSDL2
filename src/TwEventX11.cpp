...
static int _XButtonEvent(XEvent *event)
{
    ...
    return TwMouseButton(action, (TwMouseButtonID)xbe->button);
}
...
TW_API int TW_CDECL_CALL TwEventX11(void *xevent)
{
    ...
    {
    case KeyPress:    
        return _XKeyPress((XEvent*)xevent);
    ...
    case ButtonPress: 
    case ButtonRelease: 
        return _XButtonEvent((XEvent*)xevent);

    case MotionNotify:
        return _XMotionEvent((XEvent*)xevent);

    case ConfigureNotify:
        return _XConfigureEvent((XEvent*)xevent);
